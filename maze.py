...
class Maze:
	...
	def __init__(self, parent, size = 15, canvasSize = 600):
		...
		self.tiles = [[Tile(self.canvas, x, y, self.canvasSize / self.size, random.choice(list(tileTypes))) for y in range(0, size)] for x in range(0, size)]
	...
	def onMouseClick(self, event):
		...
		print("X - {} | Y - {}".format(listX, listY))
		try:
			if listX >= 0 and listY >= 0:
				self.tiles[listX][listY].setColour("purple", "purple")
		except IndexError:
			pass
