...
class Generator:
    ...
    def generate(self):
        ...
        while len(self.edges) != 0:
            ...
            for i in range(len(self.sets)):
                if self.xySet is not None and self.newxySet is not None:
                    break
            ...
            if self.xySet == self.newxySet:
                ...
                self.sets[self.xySet] = self.sets[self.xySet] + \
                    self.sets[self.newxySet]
    ...
    def connectTiles(self, tile, direction):
        ...
        xDir = direction[0]
        yDir = direction[1]
        self.maze.tiles[tile.x+(xDir // 2)][tile.y+(yDir // 2)].setPath()
        self.maze.tiles[tile.x+(xDir)][tile.y+(yDir)].setPath()
