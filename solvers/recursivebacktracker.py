...
class Solver(SolverTemplate):
    ...
    def step(self, force=False):
        if not self.autorun and not force:
            return
        # Get the current tiles neighbours, not including walls or tiles that
        # have already been visited.
        tileNeighbours = self.currentTile.findNeighbours(
            distance=1,
            blockVisited=True,
            blockWalls=True)
        ...
        if self.end in tileNeighbours:
            ...
            self.solved = True
            # Loop through the remaining stack and change all tiles to
            # the FOUNDPATH type.
        ...
        # If there are unvisited neighbours, add the current tile to the stack
        ...
        # If there are no unvisited neighbours, set the current tile as visited
        # and pop the next tile off the stack to go backwards.
        else:
            self.currentTile.setVisited()
            try:
                self.currentTile = self.stack.pop()

            # If we cannot pop an item then we have gone back to the start
            # of the stack and the maze cannot be solved.
            except IndexError:
